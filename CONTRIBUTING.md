# Contributing to Pharos MCP Server

Thank you for your interest in contributing to Pharos MCP Server! This document provides guidelines and instructions for contributing.
...
3. **Add the upstream remote**:
   ```bash
   git remote add upstream https://TBD-NEW-REMOTE
   ```
...
- [ ] Test pulling and running the new image:
  ```bash
  podman pull quay.io/geored/pharos:0.2.0
  podman run --rm quay.io/geored/pharos:0.2.0 python --version
  ```
...
podman pull quay.io/geored/pharos:latest

# Specific version
podman pull quay.io/geored/pharos:1.0.0

# Specific minor version (gets latest patch)
podman pull quay.io/geored/pharos:1.0

# Specific commit
podman pull quay.io/geored/pharos:a1b2c3d
```
...
## Characterization suite (phase 0)

`tests/characterization/` freezes current tool behavior as golden JSON.
- Run: `pytest tests/characterization/ -q`
- Regenerate after an INTENDED behavior change: `UPDATE_GOLDENS=1 pytest tests/characterization/ -q`,
  then inspect the diff — a golden diff IS a behavior change and must be called out in the PR.
- Registry artifact: `python scripts/dump_tool_registry.py` refreshes
  `parity_reference.json`; the parity test fails on any tool/schema drift.
- Live tool matrix: `python scripts/live_matrix.py run` spawns the MCP server
  as a stdio subprocess and sweeps all 48 tools against a live cluster, writing
  JSON records to `live-matrix-runs/` (gitignored). Four subcommands: `run`
  (sweep), `diff RUN-A RUN-B` (compare two runs), `bless RUN` (pin a run as
  the baseline), and `list` (show available runs). Control the storage root via
  `LIVE_MATRIX_ROOT`. A run can only be blessed if it is complete (no `--tools`
  filter) and recorded no transport failure; `diff` prints a warning when a run
  carries a transport-failure flag but never lets it affect the exit code.

## Questions?

- Open a [GitHub Discussion](https://TBD-NEW-REMOTE/discussions) for questions
