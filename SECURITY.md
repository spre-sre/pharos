...
We take the security of Pharos MCP Server seriously. If you discover a security vulnerability, please report it responsibly.
...
1. **GitHub Security Advisories** (Preferred)
   - Go to the [Security Advisories](https://TBD-NEW-REMOTE/security/advisories) page
...
Pharos MCP Server tools are designed to be **read-only**. They query and analyze Kubernetes resources but do not modify cluster state. This architectural decision significantly reduces the attack surface.
...
The canonical, tiered read-only RBAC manifest is the primary source of truth for
production and staging deployments:
[`deploy/rbac-readonly.yaml`](deploy/rbac-readonly.yaml).  It covers the tiered
resource structure (core, Tekton, OpenShift, Konflux, and the opt-in secrets tier),
includes the cluster-monitoring-view requirement for Prometheus access, and
documents the secrets blast-radius decision.  Apply instructions and the
namespace-scoped Role variant for KubeArchive TLS secrets are inline in that file.

The local-development authentication path is operator-token-based: set
`KUBEARCHIVE_TOKEN` to a bearer token you mint out-of-band.  The
self-provisioning path (`_create_local_dev_token`) that previously applied
a ClusterRole via `kubectl` has been removed.
...
We thank the security researchers and community members who help keep Pharos MCP Server secure.
